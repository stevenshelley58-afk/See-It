...
create table shop (
  ...
  access_scopes text[],
  plan text check (plan in ('trial','starter','growth','cancelled')) default 'trial',
  trial_ends_at timestamptz,
  renders_quota integer default 50,
  lifestyle_images_quota integer default 10,
...
create table product_setup (
  ...
  cutout_key text,
  prep_status text check (prep_status in ('none','extracting','awaiting_confirm','ready','failed')) default 'none',
...
create table prompt_bundle_version (
  id uuid primary key default gen_random_uuid(),
...
create table render_recipe_version (
  id uuid primary key default gen_random_uuid(),
...
create table render_trace_event (
  id uuid primary key default gen_random_uuid(),
  trace_id text not null,
  render_request_id uuid references render_request(id),
  render_attempt_id uuid references render_attempt(id),
...
create table event_log (
  ...
  surface text check (surface in ('widget','admin','demo','outreach','billing','system','founder','ai')),
  name text not null,
  shop_id uuid,
  prospect_id uuid,
...
create table eval_dataset (id uuid primary key default gen_random_uuid(), name text unique not null, description text, status text check (status in ('active','archived')), created_at timestamptz default now(), updated_at timestamptz default now());
create table eval_case (id uuid primary key default gen_random_uuid(), eval_dataset_id uuid references eval_dataset(id), case_slug text not null, product_asset_key text, cutout_asset_key text, room_asset_key text, mask_asset_key text, expected_json jsonb default '{}', notes text, created_at timestamptz default now(), unique(eval_dataset_id, case_slug));
create table eval_run (id uuid primary key default gen_random_uuid(), eval_dataset_id uuid references eval_dataset(id), name text, render_recipe_version_id uuid references render_recipe_version(id), model_route_policy_id uuid references model_route_policy(id), status text check (status in ('queued','running','completed','failed','cancelled')), summary_json jsonb default '{}', created_by text, created_at timestamptz default now(), completed_at timestamptz);
create table eval_result (id uuid primary key default gen_random_uuid(), eval_run_id uuid references eval_run(id), eval_case_id uuid references eval_case(id), render_request_id uuid references render_request(id), automated_score_json jsonb default '{}', manual_score_json jsonb default '{}', status text check (status in ('pass','fail','review')), created_at timestamptz default now());
create table ai_experiment (id uuid primary key default gen_random_uuid(), name text not null, type text check (type in ('prompt_version_test','model_test','recipe_test','gate_threshold_test','fallback_policy_test','parameter_test')), surface text not null, status text check (status in ('draft','running','paused','completed','archived')), start_at timestamptz, end_at timestamptz, traffic_percent integer default 0, success_metric text, guardrail_json jsonb default '{}', created_by text, created_at timestamptz default now(), updated_at timestamptz default now());
create table ai_experiment_arm (id uuid primary key default gen_random_uuid(), experiment_id uuid references ai_experiment(id), name text not null, render_recipe_version_id uuid references render_recipe_version(id), ai_model_id uuid references ai_model(id), prompt_bundle_version_id uuid references prompt_bundle_version(id), params_override_json jsonb default '{}', traffic_weight integer default 0, status text check (status in ('active','paused','archived')), created_at timestamptz default now());
create table ai_experiment_assignment (id uuid primary key default gen_random_uuid(), experiment_id uuid references ai_experiment(id), arm_id uuid references ai_experiment_arm(id), assignment_key text not null, render_request_id uuid references render_request(id), created_at timestamptz default now(), unique(experiment_id, assignment_key));
create table usage_monthly (shop_id uuid references shop(id), month text, renders_started integer default 0, renders_accepted integer default 0, renders_failed integer default 0, lifestyle_images_used integer default 0, cost_estimate_usd numeric default 0, created_at timestamptz default now(), updated_at timestamptz default now(), primary key(shop_id, month));
create table prospect (id uuid primary key default gen_random_uuid(), store_domain text unique, store_name text, contact_email text, email_verified boolean default false, owner_name text, hero_product_json jsonb, personalization_line text, score text check (score in ('A','B','C')), status text check (status in ('queued','demo_built','approved','sent','clicked','trial','customer','rejected','bounced','unsubscribed')), demo_slug text unique, demo_expires_at timestamptz, sequence_id text, batch_id text, created_at timestamptz default now(), updated_at timestamptz default now());
create table suppression (email text primary key, reason text check (reason in ('unsub','bounce','manual','customer')), created_at timestamptz default now());
