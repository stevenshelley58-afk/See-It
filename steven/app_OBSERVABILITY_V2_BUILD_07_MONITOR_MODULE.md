# Step 07: Monitor Module (Read Queries)

## Context

You are working on a Shopify Remix app. You have created the telemetry module for writes. Now create the monitor module for reads.

## Task

Create the monitor module with query functions and response types.

## Instructions

1. Create directory `app/services/monitor/`

2. Create `app/services/monitor/types.ts`:
...
3. Create `app/services/monitor/queries.server.ts`:
...
4. Create `app/services/monitor/index.ts`:
...
Test import:
```typescript
import { getRuns, getRunDetail } from "~/services/monitor";
```

## Do Not

- Do not write to the database from this module
- Do not throw on missing data (return null or empty arrays)
