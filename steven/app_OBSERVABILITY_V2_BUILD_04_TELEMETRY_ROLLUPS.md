# Step 04: Telemetry Rollups

## Context

You are working on a Shopify Remix app. You have created the telemetry emitter. Now create the rollup writers for RenderRun and VariantResult.

## Task

Create functions to write RenderRun and VariantResult records.

## Critical Rule

**This module must NEVER throw exceptions.** All functions must be wrapped in try/catch. If a write fails after run creation, set telemetryDropped=true on best effort.

## Instructions

1. Create `app/services/telemetry/rollups.server.ts`:
 ...
 * Returns true on success, false on failure.
 */
export async function recordVariantStart(input: RecordVariantStartInput): Promise<boolean> {
  try {
 ...
 * Returns true on success, false on failure.
 */
export async function recordVariantResult(input: RecordVariantResultInput): Promise<boolean> {
  try {
...
- File exists at `app/services/telemetry/rollups.server.ts`
