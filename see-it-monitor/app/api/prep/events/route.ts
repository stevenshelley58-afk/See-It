 ...
 * Accepts prep events from the main app (server-to-server)
 *
 * NOTE: This route must live under `app/api/...` for the deployed monitor.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      processed: successCount,
      total: processedEvents.length,
      errors: validationErrors.length > 0 || dbErrors.length > 0 ? [...validationErrors, ...dbErrors] : undefined,
    });
  } catch (error) {
    console.error('[Prep Events API] Error processing request:', error);
    return NextResponse.json(
