...
export async function POST(request: NextRequest) {
  try {
    ...
    // Read all drizzle migrations in order (0000_*.sql, 0001_*.sql, ...)
    ...
    if (migrationFiles.length === 0) {
      return NextResponse.json(
        { error: 'No migration files found in drizzle/' },
        { status: 500 }
      );
    }
    ...
    for (const file of migrationFiles) {
      ...
      try {
        ...
        return NextResponse.json(
          { error: `Failed to read migration file ${file}: ${error instanceof Error ? error.message : 'Unknown error'}` },
          { status: 500 }
        );
      }
