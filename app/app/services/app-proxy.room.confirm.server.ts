...
import { downloadWithSizeLimit } from "../utils/image-download.server";
...
export const action = async ({ request }: ActionFunctionArgs) => {
    ...
    // Fail-hard: original room image key must exist (no legacy key construction)
    if (!roomSession.originalRoomImageKey) {
        return json(
            { error: "no_original_room_image_key", message: "Room image key missing. Upload again." },
            { status: 422, headers: corsHeaders }
        );
    }
    const originalKey = roomSession.originalRoomImageKey;
    ...
    try {
        ...
        const originalBuffer = await downloadWithSizeLimit(originalUrl, MAX_ORIGINAL_DOWNLOAD_BYTES);
