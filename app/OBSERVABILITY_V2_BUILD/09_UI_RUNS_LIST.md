# Step 09: UI - Runs List Page

## Context

You are working on a Shopify Remix app. You have created the API routes. Now create the runs list UI page.

## Task

Create the monitor runs list page with filters and polling.

## Instructions

1. Create `app/routes/app.monitor._index.tsx`:
...
import { useLoaderData, useSearchParams, useNavigate, useRevalidator } from "@remix-run/react";
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  return json({
    ...result,
    versions: versions.map((v) => v.promptPackVersion),
  });
};

export default function MonitorRunsPage() {
  const { runs, total, page, pages, versions } = useLoaderData<typeof loader>();
  ...
  const [versionFilter, setVersionFilter] = useState<string[]>(
    searchParams.get("promptVersion") ? [searchParams.get("promptVersion")!] : []
  );
        ...
        <Layout.Section>
          <Card>
            ...
            <DataTable
              ...
              headings={["Time", "Product", "Status", "Variants", "Duration", "Version", ""]}
              rows={rows}
            />
...
4. If there are existing runs in the database, they should appear
5. Page should refresh every 2 seconds (check network tab)
6. Filters should work

## Do Not

- Do not delete the old `app.monitor.tsx` yet (that's in cleanup step)
- Do not add external chart libraries yet
