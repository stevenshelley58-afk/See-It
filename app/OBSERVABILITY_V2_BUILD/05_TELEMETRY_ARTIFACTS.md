# Step 05: Telemetry Artifacts

## Context

You are working on a Shopify Remix app. You have created the telemetry emitter and rollups. Now create the artifact manager for GCS storage.

## Task

Create functions to store artifacts in GCS and index them in MonitorArtifact.

## Critical Rule

**This module must NEVER throw exceptions on the hot path.** Artifact storage is optional - failures are logged and swallowed.

## Instructions

1. Read the existing storage service at `app/services/storage.server.ts` to understand the GCS helper pattern.

2. Create `app/services/telemetry/artifacts.server.ts`:
 ...
 * NEVER throws.
 */
export async function storeArtifact(input: ArtifactInput): Promise<string | null> {
  try {
...
- File exists at `app/services/telemetry/artifacts.server.ts`
- No TypeScript errors
- StorageService import resolves correctly

## Do Not

- Do not throw any exceptions
- Do not add new GCS dependencies (use existing StorageService)
