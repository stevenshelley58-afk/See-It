# Step 03: Telemetry Emitter

## Context

You are working on a Shopify Remix app. You have created constants and types for the telemetry module. Now create the event emitter.

## Task

Create the event emitter that writes to the MonitorEvent table.

## Critical Rule

**This module must NEVER throw exceptions.** All functions must be wrapped in try/catch. Failures are logged to console and swallowed. This protects the render path from observability failures.

## Instructions

1. Create `app/services/telemetry/emitter.server.ts`:
...
export function emit(input: TelemetryEventInput): void {
  doEmit(input).catch((error) => {
    console.error("[Telemetry] Failed to emit event:", input.type, error?.message || error);
  });
}
 ...
 */
export async function emitAsync(input: TelemetryEventInput): Promise<boolean> {
  try {
    await doEmit(input);
    return true;
  } catch (error) {
...
- File exists at `app/services/telemetry/emitter.server.ts`
- No TypeScript errors
- Test in isolation:
  ```typescript
  // Temporary test - remove after verification
  import { emit } from "./emitter.server";
  emit({
    shopId: "test",
    requestId: "test",
    source: "renderer",
...
- Do not await emit() on critical paths
